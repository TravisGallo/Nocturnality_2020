...
sd_mat[var_mat == Inf | var_mat == "NaN"] <- NA

# find average housing density for each city
sitecovs <- readRDS("./Data/2019-03-18_sitecovsBuffered.rds")
...
var_mat_ordered <- round(var_mat[,ord],2)
sd_mat_ordered <- round(sd_mat[,ord],2)

# manually remove cottontail data from Long Beach
rr_mat_ordered["e. cottontail","LBCA"] <- NA
var_mat_ordered["e. cottontail","LBCA"] <- NA
sd_mat_ordered["e. cottontail","LBCA"] <- NA

# matrix for labeling
label_mat <- matrix(paste0(rr_mat_ordered," (",sd_mat_ordered,")"),
               nrow = nrow(rr_mat_ordered), ncol = ncol(rr_mat_ordered))
label_mat[label_mat == "NA (NA)"] <- ""
...
svg("rr_base.svg", width = 10, height = 10, bg = "white")
